#' get_par() is used for automatically downloadiing, decompressing and renaming
#' sea photosynthetically available radiation data by custom grid size, time
#' span and satellite.
#'
#' @title get_par
#' @author Chao Xu
...
get_par <- function(file.path,
  ...
  format <- 'hdf/'
  sate <- ifelse(satellite == 'MODIS', 'par.modis.r2018/',
                  ifelse(satellite == 'VIIRS', 'par.viirs.r2018/', NULL))
  ...
  name <- filename %>% str_extract('[[:digit:]]+') %>%
    ...
    group_by(name) %>%
    mutate(name1 = paste0('par.', name, '.hdf', '.gz'),
