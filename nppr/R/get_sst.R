#' get_sst() is used for automatically downloadiing, decompressing and renaming
#' sea surface temperature data by custom grid size, time
#' span and satellite.
#'
#' @title get_sst
#' @author Chao Xu
...
get_sst <- function(file.path,
  ...
  format <- 'hdf/'
  sate <- ifelse(satellite == 'MODIS', 'sst.modis.r2018/',
                  ifelse(satellite == 'VIIRS', 'sst.viirs.r2018/', NULL))
  ...
  name <- filename %>% str_extract('[[:digit:]]+') %>%
    ...
    group_by(name) %>%
    mutate(name1 = paste0('sst.', name, '.hdf', '.gz'),
